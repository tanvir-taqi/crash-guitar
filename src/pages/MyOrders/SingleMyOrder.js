...
const SingleMyOrder = ({myOrder,i,handleDeleteOrder}) => {
    const {price, sellerEmail,status, sellerName,productId, productName, buyersName, buyersEmail, phone, location, productPhoto, sellerLocation, sellersPhone ,_id} = myOrder
console.log(status);
    return (
        <div className='grid grid-cols-1 md:grid-cols-6 gap-6 items-center md:rounded-full w-full md:flex-row bg-cyan-200 my-4 p-3  '>
        <p className='font-extrabold mx-3'>{i+1}</p>
